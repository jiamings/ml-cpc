...
class CPCLoss(nn.Module):
    def __init__(self, opt):
        ...
        self.contrast = ContrastMemory(
            opt.feat_dim, opt.n_data, opt.nce_k, opt.nce_t, opt.nce_m)
        self.criterion_t = _MultiClassLoss(opt.n_data, opt.cpc_alpha)
        self.criterion_s = _MultiClassLoss(opt.n_data, opt.cpc_alpha)
...
class _MultiLabelLoss(nn.Module):
    ...
    def forward(self, x):
        ...
        log_denominator = ((P_pos * alpha).mean() +
...
class _MultiClassLoss(nn.Module):
    def __init__(self, n_data, alpha=1.0):
        super(_MultiClassLoss, self).__init__()
        self.n_data = n_data
        self.alpha = alpha
    ...
    def forward(self, x):
        ...
        P_pos = x.select(1, 0) * alpha
        P_neg = x.narrow(1, 1, m) * beta

        log_denominator = (P_pos.sum(dim=1) + P_neg.sum(dim=1)).div(m+1).log()
